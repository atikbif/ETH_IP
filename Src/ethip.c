...
static uint16_t class0x90_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
	  ,
	  ...
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0xa2, 0x6d,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe,
	  0x00, 0xfe, 0x00, 0xfe, 0x00, 0xfe, 0x00, 0x00,
	  0x03, 0x06, 0x13, 0xac, 0x00, 0x10, 0x36, 0x59,
	  0x20, 0x36, 0x54, 0x20, 0x4d, 0x43, 0x20, 0x32,
	  0x42, 0x4d, 0x2f, 0x75, 0x6f, 0x37
	};
	static uint16_t i = 0;
	uint16_t offset = create_rr_data_reply_header(out,1,0,4+sizeof(answer));
	// data CIP
	// response
	out[offset++] = 0x8E;
	// reserved shal be zero
	...
	// size of additional status
	out[offset++] = 0x00;
	for(i=0;i<sizeof(answer);i++) out[offset++] = answer[i];
	return offset;
}

static uint16_t class0x91_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
	  0xaa, 0xaa, 0x3a, 0xa3, 0xaa, 0xaa, 0xaa, 0xaa,
...
static uint16_t class0x92_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
		0xaa, 0xaa, 0xa1, 0x18, 0xaa, 0xaa, 0xaa, 0xaa,
...
static uint16_t class0x93_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
		0x54, 0xa5, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
		0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa
	};
	static uint16_t i = 0;
	uint16_t offset = create_rr_data_reply_header(out,1,0,4+sizeof(answer));
...
static uint16_t class0x94_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
		...
		0x00, 0x00, 0x00, 0x60, 0x18, 0x08, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00
	};
	static uint16_t i = 0;
...
static uint16_t class0x95_get_attribute_single(uint8_t* out) {
	static uint8_t answer[] = {
		
	};
...
static uint16_t get_attribute_single(uint8_t* out,uint8_t class_num, uint8_t impl_num, uint8_t attr_num) {
	...
	if(class_num==0x9C && impl_num==0x01 && attr_num==0x01) return class0x9c_get_attribute_single(out);
	return 0;
}
